...
public class FastNoiseLite {
   ...
   public void setDomainWarpFreq(float domainWarpFreq) {
      this.mDomainWarpFreq = domainWarpFreq;
   }
      ...
      switch (this.mNoiseType) {
         ...
         default:
            return switch (this.mFractalType) {
               case FBm -> this.GenFractalFBm(x, y);
               case Ridged -> this.GenFractalRidged(x, y);
               case PingPong -> this.GenFractalPingPong(x, y);
               default -> this.GenNoiseSingle(this.mSeed, x, y);
            };
      }
   }
   ...
   public float getNoise(double x, double y, double z) {
      ...
      return switch (this.mFractalType) {
         case FBm -> this.GenFractalFBm(x, y, z);
         case Ridged -> this.GenFractalRidged(x, y, z);
         case PingPong -> this.GenFractalPingPong(x, y, z);
         default -> this.GenNoiseSingle(this.mSeed, x, y, z);
      };
   }
   ...
   private float GenNoiseSingle(int seed, double x, double y) {
      return switch (this.mNoiseType) {
         case OpenSimplex2 -> this.SingleSimplex(seed, x, y);
         case OpenSimplex2S -> this.SingleOpenSimplex2S(seed, x, y);
         case Cellular -> this.SingleCellular(seed, x, y);
         case Perlin -> this.SinglePerlin(seed, x, y);
         case ValueCubic -> this.SingleValueCubic(seed, x, y);
         case Value -> this.SingleValue(seed, x, y);
      };
   }

   private float GenNoiseSingle(int seed, double x, double y, double z) {
      return switch (this.mNoiseType) {
         case OpenSimplex2 -> this.SingleOpenSimplex2(seed, x, y, z);
         case OpenSimplex2S -> this.SingleOpenSimplex2S(seed, x, y, z);
         case Cellular -> this.SingleCellular(seed, x, y, z);
         case Perlin -> this.SinglePerlin(seed, x, y, z);
         case ValueCubic -> this.SingleValueCubic(seed, x, y, z);
         case Value -> this.SingleValue(seed, x, y, z);
      };
   }
   ...
   public Vector3d pointFor(int seed, double jitter, double x, double y, double z) {
      ...
      int yr = fastRound(y);
      int zr = fastRound(z);
      int hash = hash(seed, xr, yr, zr);
      int idx = hash & 1020;
      double vecX = randVecs3D[idx] * jitter + xr;
      double vecY = randVecs3D[idx | 1] * jitter + yr;
      double vecZ = randVecs3D[idx | 2] * jitter + zr;
   ...
   public double pointFor(int seed, double jitter, double x) {
      int xr = fastRound(x);
      int hash = hash(seed, xr, 0);
      int idx = hash & 1020;
      return randVecs3D[idx] * jitter + xr;
   }
   ...
   public float SingleCellular(int seed, double x, double y) {
      ...
      int closestHash = 0;
      float cellularJitter = 0.5F * this.mCellularJitterModifier;
      ...
      return switch (this.mCellularReturnType) {
         case CellValue -> closestHash * 4.656613E-10F;
         case Distance -> distance0 - 1.0F;
         case Distance2 -> distance1 - 1.0F;
         case Distance2Add -> (distance1 + distance0) * 0.5F - 1.0F;
         case Distance2Sub -> distance1 - distance0 - 1.0F;
         case Distance2Mul -> distance1 * distance0 * 0.5F - 1.0F;
         case Distance2Div -> distance0 / distance1 - 1.0F;
      };
   }
   ...
   private void DomainWarpSingle(@Nonnull FastNoiseLite.Vector2 coord) {
      ...
      float freq = this.mDomainWarpFreq;
      double xs = coord.x;
      double ys = coord.y;
      switch (this.mDomainWarpType) {
         case OpenSimplex2:
         case OpenSimplex2Reduced:
            double SQRT3 = 1.7320508075688772;
            double F2 = 0.3660254037844386;
            double t = (xs + ys) * 0.3660254037844386;
            xs += t;
            ys += t;
         default:
   ...
   private void SingleDomainWarpSimplexGradient(
      int seed, float warpAmp, float frequency, double x, double y, @Nonnull FastNoiseLite.Vector2 coord, boolean outGradOnly
   ) {
      float SQRT3 = 1.7320508F;
      float G2 = 0.21132487F;
      x *= frequency;
   ...
   private void SingleDomainWarpOpenSimplex2Gradient(
      int seed, float warpAmp, float frequency, double x, double y, double z, @Nonnull FastNoiseLite.Vector3 coord, boolean outGradOnly
   ) {
      x *= frequency;
      y *= frequency;
      z *= frequency;
      ...
      while (true) {
         ...
         if (var56 > 1.0F) {
            float bbbb = --var56 * var56 * (var56 * var56);
            float xo;
            float yo;
            float zo;
            if (outGradOnly) {
